...
def _common_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="运行时数据目录（默认取 PERSONAL_AGENT_DATA_DIR）",
    )
    parser.add_argument(
        "--database", type=Path, default=None, help="知识库 SQLite 路径（默认 data/knowledge.db）"
    )
    parser.add_argument(
        "--hash-embedding", action="store_true", help="使用哈希向量（仅供测试，不用于生产检索）"
    )
    parser.add_argument(
        "--embedding-model", default="BAAI/bge-small-zh-v1.5", help="本地 embedding 模型"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_parser.add_argument(
        "--input", type=Path, required=True, help="原始资料目录（.md/.txt/.json）"
    )
    ...
    run_parser.add_argument(
        "--incremental", action="store_true", help="增量模式（跳过内容未变化的文件）"
    )
    ...
    approve_parser.add_argument(
        "--input", type=Path, default=None, help="原始资料目录（恢复图所需）"
    )
