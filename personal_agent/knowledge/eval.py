...
def retrieve(
    ...
    *,
    ...
    return service.search_hybrid(question, limit=limit, minimum_semantic_score=minimum_semantic_score)
...
def evaluate_cases(
    ...
    *,
    ...
    for case in cases:
        matches = retrieve(
            service, strategy, case.question, limit=limit,
        ...
        results.append(
            CaseResult(
                question=case.question, expected=case.expected_source_ids,
                retrieved=retrieved_ids, hit_rank=metrics["hit_rank"], strategy=strategy,
            )
        )
...
def _report_payload(results: list[CaseResult], summaries: list[Summary], *, limit: int, top_k: list[int], minimum_semantic_score: float) -> dict:
    return {
        "limit": limit,
        "top_k": top_k,
        "minimum_semantic_score": minimum_semantic_score,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--strategy", choices=["semantic", "keyword", "hybrid", "all"], default="hybrid",
    ...
    parser.add_argument("--min-score", type=float, default=0.0, help="语义阈值（hybrid 生效；生产 0.35）")
    parser.add_argument("--embedding-model", default="BAAI/bge-small-zh-v1.5")
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--hash-embedding", action="store_true", help="仅用于测试：哈希向量无语义，指标无意义")
    ...
    if args.limit < max(top_k):
        parser.error(f"--limit（{args.limit}）应 >= max top-k（{max(top_k)}），否则高 k 值按实际返回计算")
    ...
    try:
        ...
        for strategy in strategies:
            results, summary = evaluate_cases(
                service, eval_set.cases, strategy,
                limit=args.limit, minimum_semantic_score=args.min_score, top_k=top_k,
            )
    ...
    if args.report:
        payload = _report_payload(all_results, summaries, limit=args.limit, top_k=top_k, minimum_semantic_score=args.min_score)
