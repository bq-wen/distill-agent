...
from personal_agent.application.quota import TokenQuotaStore
...
from personal_agent.contracts import HealthResponse, ProfileResponse, QuotaResponse, StatusResponse, TopicGroup
...
def create_app(
    *,
    ...
    status_provider: Callable[[], StatusResponse] | None = None,
) -> FastAPI:
    ...
    @app.get("/api/quota", response_model=QuotaResponse, tags=["system"])
    async def get_quota() -> QuotaResponse:
        if quota_store is None or daily_token_budget is None:
            raise HTTPException(status_code=503, detail="配额服务尚未配置")
        used = quota_store.used_today()
        return QuotaResponse(
            used_today=used,
            daily_budget=daily_token_budget,
            remaining=max(daily_token_budget - used, 0),
            exhausted=used >= daily_token_budget,
        )

    @app.get("/api/status", response_model=StatusResponse, tags=["system"])
    async def get_status() -> StatusResponse:
        if status_provider is None:
            raise HTTPException(status_code=503, detail="状态服务尚未配置")
    ...
    async def submit_message(
        ...
        if (
            ...
            and daily_token_budget is not None
            and quota_store.used_today() >= daily_token_budget
        ):
            raise HTTPException(status_code=429, detail="今日 token 预算已用尽，请明日再来")
        try:
