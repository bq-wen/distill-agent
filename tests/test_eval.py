...
def test_hybrid_merge_dedups_and_prefers_semantic_over_keyword(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert keyword_only and all(
            match.source.source_id == "wengraph-overview" for match in keyword_only
        )
...
def test_evaluate_cases_end_to_end_produces_bounded_metrics(tmp_path: Path) -> None:
    ...
    cases = [
        EvalCase(
            question="ToolGuard 如何治理工具调用？", expected_source_ids=["wengraph-overview"]
        ),
